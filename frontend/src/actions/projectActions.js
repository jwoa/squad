...
export const createProjectTask = (projectId, task) => async (dispatch, getState) => {
    try {
        dispatch({
            type: PROJECT_CREATE_TASK_REQUEST
        })
        ...
        await axios.post(`/api/projects/${projectId}/tasks`, task, config)

        dispatch({
            type: PROJECT_CREATE_TASK_SUCCESS
        })
